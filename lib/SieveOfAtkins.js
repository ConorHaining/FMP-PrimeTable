module.exports = {
	 ...
	 */
	primeList(noOfPrimes) {
		// Arbitrarily set a flexible limit
		var limit = noOfPrimes * 17;
		var sqrtLimit = Math.floor(Math.sqrt(limit));

		// Manually handle the smallest primes
		if(noOfPrimes == 1) {
			return [2];
		} else if (noOfPrimes == 2) {
			return [2, 3];
		} else if (noOfPrimes == 3) {
			return [2, 3, 5];
		}
